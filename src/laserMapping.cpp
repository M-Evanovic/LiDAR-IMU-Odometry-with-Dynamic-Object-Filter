...
#include "ground_seperator.hpp"
#include "dyn_obj_filter/dyn_obj_filter.hpp"
...
PointCloudXYZI::Ptr ori_feats_undistort(new PointCloudXYZI());
...
PointCloudXYZI::Ptr ground_body{new PointCloudXYZI()};
PointCloudXYZI::Ptr non_ground_body{new PointCloudXYZI()};
...
//------------------------------------------------------------------------------------------------------
    ...
    while (status)
    {
        ...
        if(sync_packages(Measures)) 
        {
            ...
            ground_seperator->EstimateGround(*feats_undistort, *ground_body, *non_ground_body);

            Quatd curr_rot_q(state_point.rot.coeffs()[3], state_point.rot.coeffs()[0], state_point.rot.coeffs()[1], state_point.rot.coeffs()[2]);
            M3D curr_rot = curr_rot_q.toRotationMatrix();
            V3D curr_pos(state_point.pos(0), state_point.pos(1), state_point.pos(2));

            do_filter->Process(*non_ground_body, curr_rot, curr_pos, *static_pc, *dynamic_pc);
            *ori_feats_undistort = *feats_undistort;
            *static_pc += *ground_body;
            *feats_undistort = *static_pc;
