#ifndef GROUNDSEPERATOR_H
#define GROUNDSEPERATOR_H
...
public:
    ...
    GroundSeperator(Options option = Options()) : option_(option)
    {
        // init params
        Initialization();
    }   
...
template <typename PointT>
inline void GroundSeperator<PointT>::InitializeZone(Zone &z, int num_sectors, int num_rings)
{
    z.clear();
...
template <typename PointT>
inline void GroundSeperator<PointT>::FlushPatchesInZone(Zone &patches, int num_sectors, int num_rings)
{
    for (int i = 0; i < num_sectors; i++)
    {
        for (int j = 0; j < num_rings; j++)
...
template <typename PointT>
inline void GroundSeperator<PointT>::FlushPatches(std::vector<Zone> &czm)
{
...
template <typename PointT>
inline void GroundSeperator<PointT>::EstimatePlane(const pcl::PointCloud<PointT> &ground)
{
...
template <typename PointT>
inline void GroundSeperator<PointT>::ExtractInitialSeeds(
    const int zone_idx, const pcl::PointCloud<PointT> &p_sorted,
    pcl::PointCloud<PointT> &init_seeds, double th_seed)
{
...
template <typename PointT>
inline void GroundSeperator<PointT>::ReflectedNoiseRemoval(pcl::PointCloud<PointT> &cloud_in, pcl::PointCloud<PointT> &cloud_nonground)
{
...
template <typename PointT>
inline void GroundSeperator<PointT>::EstimateGround(
...
template <typename PointT>
double GroundSeperator<PointT>::CalculateCosineSimilarity(pcl::PointXYZINormal &pt1, pcl::PointXYZINormal &pt2) {
...
template <typename PointT>
pcl::PointCloud<pcl::PointXYZINormal> GroundSeperator<PointT>::RemoveDifferentPlanes(pcl::PointCloud<pcl::PointXYZINormal> &normals) {
...
template <typename PointT>
Eigen::VectorXf GroundSeperator<PointT>::FitGroundPlane(pcl::PointCloud<pcl::PointXYZINormal> &normals) {
...
template <typename PointT>
Eigen::VectorXf GroundSeperator<PointT>::FitRansacGroundPlane(pcl::PointCloud<PointT> &cloud) {
...
template <typename PointT>
inline void GroundSeperator<PointT>::UpdateElevationThr(void) {
...
template <typename PointT>
inline void GroundSeperator<PointT>::UpdateFlatnessThr(void) {
    for (int i = 0; i < num_rings_of_interest_; i++) {
...
template <typename PointT>
inline void GroundSeperator<PointT>::TemporalGroundRevert(pcl::PointCloud<PointT> &cloud_ground, pcl::PointCloud<PointT> &cloud_nonground,
                                                        std::vector<double> ring_flatness, std::vector<RevertCandidate<PointT>> candidates,
                                                        int concentric_idx) {
    if (option_.verbose) {
        std::cout << "\033[1;34m"
...
// For adaptive
template <typename PointT>
inline void GroundSeperator<PointT>::ExtractPiecewiseGround(
    const int zone_idx, const pcl::PointCloud<PointT> &src,
    pcl::PointCloud<PointT> &dst,
    pcl::PointCloud<PointT> &non_ground_dst) {
...
template <typename PointT>
inline void GroundSeperator<PointT>::CalcMeanStdev(std::vector<double> vec, double &mean, double &stdev) {
    if (vec.size() <= 1)
        return;
...
template <typename PointT>
inline double GroundSeperator<PointT>::XY2Theta(const double &x, const double &y) { // 0 ~ 2 * PI
    // if (y >= 0) {
...
template <typename PointT>
inline double GroundSeperator<PointT>::XY2Radius(const double &x, const double &y) {
    return sqrt(pow(x, 2) + pow(y, 2));
}

template <typename PointT>
inline void GroundSeperator<PointT>::PC2CZM(const pcl::PointCloud<PointT> &src, std::vector<Zone> &czm, pcl::PointCloud<PointT> &cloud_nonground) {
